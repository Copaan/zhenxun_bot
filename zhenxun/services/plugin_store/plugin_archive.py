...
from zhenxun.services.plugin_store import plugin_store_transaction as transaction
from zhenxun.services.plugin_store.plugin_archive_dependencies import (
    archive_environment_fingerprint as _archive_environment_fingerprint,
)
from zhenxun.services.plugin_store.plugin_archive_metadata import (
    MetadataError,
    declared_package_paths,
    poetry_dependencies,
)
from zhenxun.services.plugin_store.plugin_store_receipts import (
    StoreReceiptStore,
    source_digest,
)
...
class ArchiveError(ValueError):
    def __init__(self, code: str, status: int = 400, *, candidates=None, details=None):
        super().__init__(code)
        self.code = code
        self.status = status
...
async def inspect_in_worker(path: Path, kind: str) -> dict[str, Any]:
    ...
    job = await _start_process(
        ...
        sys.executable,
        "-m",
        "zhenxun.services.plugin_store.plugin_archive_worker",
        str(path.resolve()),
        kind,
...
async def dependency_plan(
            ...
            if name in pins and pins[name] != version:
                raise ArchiveError(
                    ...
                    details={
                        ...
                        "expected": version,
                        "actual": pins[name],
                        "conflict_source": "archive_or_pending_pin",
                    },
                )
            pins[name] = version
    ...
    if any(
        ...
        raise ArchiveError(
            ...
            409,
            details={
                "conflict_source": "resolved_archive_dependencies",
                "archive_packages": pins,
...
def dependency_fingerprint() -> str:
    from zhenxun.services.nonebot_store.dependencies import environment_fingerprint
    from zhenxun.services.nonebot_store.storage import (
        pending_transaction as nonebot_pending,
    )
    from zhenxun.services.plugin_store.plugin_archive_dependencies import (
...
def confirm(
    ...
    if existing:
        ...
        receipt = {
            ...
            "environment_fingerprint": _archive_environment_fingerprint(),
